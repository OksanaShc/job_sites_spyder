...
class RabotaUaLogin(Base):
    ...
    LOGIN_FIELD = RABOTAUA['login']
    PASSWORD_FIELD = RABOTAUA['password']
...
def run_rabota_ua():
    start = time.time()
    print('Start scrapping RabotaUA')
    c = RabotaUAManager(keyword=RABOTAUA['keyword'], worker=worker_runner, read_contacts=True)
    c.process()
    c.write_file()
    end = time.time()
    print('Finished RabotaUA scrapping. Total time: %s' % round(end - start))
